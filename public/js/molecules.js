 ...
 * Jeff Daze
 * Oct, 4th 2015
 * This is shamelessly modified from the three.js example...
 * some subtle changes but ultimately based on the original from here:
 *
 *  threejs.org/examples/css3d_molecules.html
 *
 * Still shocked that three.js has a PDB loader!!
 * huzzah!
 *
 * Use the overlay div to put things over the rotating molecule
 *
 */
...
function init() {
	...
	camera = new THREE.PerspectiveCamera( 70, (window.innerWidth/2.5) / (window.innerHeight/1.8), 1, 5000 );
	//lower z makes the molecule appear larger!
	...
	renderer.setSize( (window.innerWidth/2.5), (window.innerHeight/1.8) );
...
function loadMolecule( url ) {
	...
	loader.load( url, function ( geometry, geometryBonds ) {
		...
		//this is retained from the original example but never used...
...
function onWindowResize() {
	...
	camera.aspect = (window.innerWidth/2.5) / (window.innerHeight/1.8);
